...
<html lang = "ko">
    ...
    <meta charset="UTF-8">
        <title></title>
        <link rel="stylesheet" href="views/style/boardDetail.css">
        <style>
    ...
    <script>
        ...
        function goList(){
        	location.href='<%= request.getContextPath() %>/freeBoard.bo?bNo=<%= b.getbNo() %>';
        }
        function updateBoard(){
        	location.href='<%= request.getContextPath() %>/boardWriteForm.up?bNo=<%= b.getbNo() %>';
        }
        function deleteBoard(){
			if(confirm('정말 삭제하시겠습니까?')){
				location.href='<%= request.getContextPath() %>/boardWrite.del?bNo=<%= b.getbNo() %>';
			}
		}
